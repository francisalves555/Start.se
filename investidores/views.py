...
def ver_empresa(request, id):
    ...
    proposta_investimanto = PropostaInvestimento.objects.filter(empresa=empresa).filter(status='PA')
    percentual_vendido = 0
    for pi in proposta_investimanto:
    ...
    return render (request, 'ver_empresa.html', {'empresa':empresa, 'documentos':documentos, 'percentual_vendido':int(percentual_vendido), 
                                                 'concretizado':concretizado, 'percentual_disponivel':percentual_disponivel})
...
def realizar_proposta(request, id):
    ...
    if valuation < (int(empresa.valuation/2)):
        messages.add_message(request, constants.WARNING, f'O seu valuantion proprosto foi R${valuation} e deve ser no minimo {empresa.valuation}')
        return redirect (f'/investidores/ver_empresa/{id}')
